...
#define KIRpong_ALLOW_GPU_SUPPORT
...
#define KIRpong_PACKED_NUM_IN_UTYPE(x)  (((x)/4) + (((x) % 4)!=0 ? 1 : 0))

/// The max number of SNP markers in an individual classifier
#define KIRpong_MAXNUM_SNP_IN_CLASSIFIER	256

/// The max number of bytes or shorts for packed SNP genotypes
#define KIRpong_PACKED_UTYPE_MAXNUM_SNP		\
	KIRpong_PACKED_NUM_IN_UTYPE(KIRpong_MAXNUM_SNP_IN_CLASSIFIER)
...
{
	UINT8 PackedHaplo[KIRpong_PACKED_UTYPE_MAXNUM_SNP];
	float Frequency;
};
...
{
	UINT8 PackedSNPs[KIRpong_PACKED_UTYPE_MAXNUM_SNP];
	int BootstrapCount;
	THLAType HLA;
};
