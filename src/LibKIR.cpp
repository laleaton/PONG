...
#  if defined(KIRpong_SYS_UNIX)
#    include <dlfcn.h>
#  elif defined(KIRpong_SYS_WIN)
#    include <windows.h>
#  else
#    error "not supported"
#  endif
...
static TFLOAT EXP_LOG_MIN_RARE_FREQ[KIRpong_MAXNUM_SNP_IN_CLASSIFIER*2];

/// the hamming distance
/// SNP genotypes (first dimension) and a pair of haplotypes (second dimension)
static UINT8 _Packed_HammingDistance[256][256] KIRpong_SSE_VAR_ALIGN;

#ifdef KIRpong_SSE_OPTIMIZE_HAMMING_DISTANCE
// 32 -- the length of genotypes
static __m128 _Packed_HamLenMask[32] KIRpong_SSE_VAR_ALIGN;
#endif
...
public:
	CInit()
	{
		for (int i=0; i < KIRpong_MAXNUM_SNP_IN_CLASSIFIER*2; i++)
...
{
	KIRpong_CHECKING((idx<0) || (idx>=KIRpong_MAXNUM_SNP_IN_CLASSIFIER),
...
{
	KIRpong_CHECKING((Length<0) || (Length>KIRpong_MAXNUM_SNP_IN_CLASSIFIER),
...
{
	KIRpong_CHECKING((int)str.size() > KIRpong_MAXNUM_SNP_IN_CLASSIFIER,
		"THaplotype::StrToHaplo, the input string is too long.");
	for (int i=0; i < (int)str.size(); i++)
	{
		char ch = str[i];
		KIRpong_CHECKING(ch!='0' && ch!='1',
...
{
	KIRpong_CHECKING(Length > KIRpong_MAXNUM_SNP_IN_CLASSIFIER,
...
void TGenotype::StringToSNP(const string &str)
{
	...
	{
		char ch = str[i];
		KIRpong_CHECKING(ch!='0' && ch!='1' && ch!='2' && ch!='?',
	...
	const THaplotype &H1, const THaplotype &H2) const
{
	...
	// UTYPE = UINT16
	static UINT16 offset[8] KIRpong_SSE_VAR_ALIGN;
...
{
	KIRpong_CHECKING(Num_SNP >= KIRpong_MAXNUM_SNP_IN_CLASSIFIER,
	...
	const TFLOAT AFreq) const
{
	...
	KIRpong_CHECKING(List.size() != OutHaplos.List.size(), msg);
	...
	for (int i=0; i < (int)List.size(); i++)
	{
		const vector<THaplotype> &src = List[i];
		vector<THaplotype> &dst = OutHaplos.List[i];
		KIRpong_CHECKING(dst.size() != src.size()*2, msg);
	...
	if (_gpuRunInDoublePrecision)
	{
		...
		for (it = List.begin(); it != List.end(); it++)
		{
			for (p = it->begin(); p != it->end(); p++)
			{
				...
				for (size_t n=KIRpong_PACKED_UTYPE_MAXNUM_SNP; n > 0; n--)
					*d ++ = *s ++;
			#else
				memcpy(&(pH->PackedHaplo[0]), &(p->PackedHaplo[0]),
					KIRpong_PACKED_UTYPE_MAXNUM_SNP);
			#endif
...
{
	KIRpong_CHECKING(nSamp() != SNPMat.Num_Total_Samp,
	...
	CHaplotypeList &NextHaplo)
{
	...
	KIRpong_CHECKING(GenoList.nSamp() != HLAList.nSamp(),
...
bool CAlg_EM::PrepareNewSNP(const int NewSNP, const CHaplotypeList &CurHaplo,
	...
	CHaplotypeList &NextHaplo)
{
	KIRpong_CHECKING((NewSNP<0) || (NewSNP>=SNPMat.Num_Total_SNP),
		"CAlg_EM::PrepareNewSNP, invalid NewSNP.");
	KIRpong_CHECKING(SNPMat.Num_Total_Samp != GenoList.nSamp(),
...
{
	KIRpong_CHECKING(n_hla<=0, "CAlg_Prediction::Init, n_hla error.");
...
{
	KIRpong_CHECKING(snpMat.Num_Total_Samp != hlaList.nSamp(),
...
TFLOAT CVariableSelection::_OutOfBagAccuracy(CHaplotypeList &Haplo)
{
	...
	KIRpong_CHECKING(Haplo.Num_SNP != _GenoList.Num_SNP,
	...
	bool verbose, bool verbose_detail)
{
	...
	while ((VarSampling.TotalNum()>0) &&
		((int)OutSNPIndex.size() < KIRpong_MAXNUM_SNP_IN_CLASSIFIER))
	{
		...
		for (it=_GenoList.List.begin(); it != _GenoList.List.end(); it++, pHLA++)
		{
			if (it->BootstrapCount <= 0)
			{
				...
				for (size_t n=KIRpong_PACKED_UTYPE_MAXNUM_SNP; n > 0; n--)
					*d ++ = *s ++;
			#else
				memcpy(p->PackedSNPs, it->PackedSNPs, KIRpong_PACKED_UTYPE_MAXNUM_SNP);
			#endif
				p->BootstrapCount = 0;
				p->HLA = *pHLA;
				p ++;
...
{
	KIRpong_CHECKING(n_snp < 0, "CAttrBag_Model::InitTraining, n_snp error.")
	KIRpong_CHECKING(n_samp < 0, "CAttrBag_Model::InitTraining, n_samp error.")
	KIRpong_CHECKING(n_hla < 0, "CAttrBag_Model::InitTraining, n_hla error.")
	...
	int n_hla, int *H1, int *H2)
{
	...
	{
		KIRpong_CHECKING(H1[i]<0 || H1[i]>=n_hla,
			"CAttrBag_Model::InitTraining, H1 error.");
		KIRpong_CHECKING(H2[i]<0 || H2[i]>=n_hla,
