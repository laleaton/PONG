...
**/
DLLEXPORT void KIRpong_SortAlleleStr(int *n_hla, char *const hlastr[],
...
DLLEXPORT void KIRpong_AlleleStrand(char *allele1[], double afreq1[], int I1[],
...
// -----------------------------------------------------------------------
//
// KIRpong: Attribute Bagging method
//

/// the number of attribute bagging models allowed
#define MODEL_NUM_LIMIT 256
/// the model container
static CAttrBag_Model* _KIRpong_MODELS_[MODEL_NUM_LIMIT];

/// need a new model
static int _Need_New_KIRpong_Model()
{
	for (int i=0; i < MODEL_NUM_LIMIT; i++)
		if (_KIRpong_MODELS_[i] == NULL) return i;
	throw ErrHLA("No memory space to store a new KIRpong model, "
		"please call \"hlaClose\" to release unused KIRpong models.");
}

/// check the model
static void _Check_KIRpong_Model(int model)
{
	if ((0 <= model) && (model < MODEL_NUM_LIMIT))
	{
		if (_KIRpong_MODELS_[model] != NULL)
			return;
	}
	throw ErrHLA("The handle of KIRpong model has been closed.");
}


/**
 *  to build a KIRpong model
 *
 ...
 *  \param out_err    output the error information, 0 -- no error, 1 -- an error exists
**/
DLLEXPORT void KIRpong_New(int *nSamp, int *nSNP, int *nHLA, int *out_Model,
	LongBool *out_err)
{
	CORETRY
		int model = _Need_New_KIRpong_Model();
		_KIRpong_MODELS_[model] = new CAttrBag_Model;
		_KIRpong_MODELS_[model]->InitTraining(*nSNP, *nSamp, *nHLA);
		*out_Model = model;
		*out_err = 0;
	CORECATCH(*out_err = 1)
}
 ...
 *  \param out_err    output the error information, 0 -- no error, 1 -- an error exists
**/
DLLEXPORT void KIRpong_Training(int *nSNP, int *nSamp, int *snp_geno, int *nHLA,
	int *H1, int *H2, int *out_Model, LongBool *out_err)
{
	CORETRY
		int model = _Need_New_KIRpong_Model();
		_KIRpong_MODELS_[model] = new CAttrBag_Model;
		_KIRpong_MODELS_[model]->InitTraining(*nSNP, *nSamp, snp_geno, *nHLA, H1, H2);
		*out_Model = model;
		*out_err = 0;
	CORECATCH(*out_err = 1)
}
...
/**
 *  to close an existing KIRpong model 
 *
 *  \param model      the model index
 *  \param out_err    output the error information, 0 -- no error, 1 -- an error exists
**/
DLLEXPORT void KIRpong_Close(int *model, LongBool *out_err)
{
	CORETRY
		_Check_KIRpong_Model(*model);
		CAttrBag_Model* m = _KIRpong_MODELS_[*model];
		_KIRpong_MODELS_[*model] = NULL;
		delete m;
		*out_err = 0;
	CORECATCH(*out_err = 1)
}
 ...
 *  \param out_err         output the error information, 0 -- no error, 1 -- an error exists
**/
DLLEXPORT void KIRpong_NewClassifiers(int *model, int *nclassifier, int *mtry,
	...
	LongBool *out_err)
{
	...
	CORETRY
		_Check_KIRpong_Model(*model);
		_KIRpong_MODELS_[*model]->BuildClassifiers(*nclassifier, *mtry,
 ...
 *  \param out_err      output the error information, 0 -- no error, 1 -- an error exists
**/
DLLEXPORT void KIRpong_Predict_Resp(int *model, int *GenoMat, int *nSamp,
	...
	int out_H1[], int out_H2[], double out_Prob[], LongBool *out_err)
{
	...
	#else
	#  error "Invalid KIRpong_FLOAT_TYPE_ID"
	#endif
 ...
 *  \param out_err      output the error information, 0 -- no error, 1 -- an error exists
**/
DLLEXPORT void KIRpong_Predict_Prob(int *model, int *GenoMat, int *nSamp,
 ...
 *  \param out_err      output the error information, 0 -- no error, 1 -- an error exists
**/
DLLEXPORT void KIRpong_Predict_Resp_Prob(int *model, int *GenoMat, int *nSamp,
	int *vote_method, LongBool *ShowInfo,
 ...
 *  \param out_err      output the error information, 0 -- no error, 1 -- an error exists
**/
DLLEXPORT void KIRpong_NewClassifierHaplo(int *model, int *n_snp, int snpidx[],
	int samp_num[], int *n_haplo, double *freq, int *hla, char *haplo[],
	double *acc, LongBool *out_err)
{
	CORETRY
		_Check_KIRpong_Model(*model);
		CAttrBag_Classifier *I = _KIRpong_MODELS_[*model]->NewClassifierAllSamp();
 ...
 *  \param out_err      output the error information, 0 -- no error, 1 -- an error exists
**/
DLLEXPORT void KIRpong_GetNumClassifiers(int *model, int *out_Num, LongBool *out_err)
{
	CORETRY
		_Check_KIRpong_Model(*model);
		*out_Num = _KIRpong_MODELS_[*model]->ClassifierList().size();
		*out_err = 0;
	CORECATCH(*out_err = 1)
}
 ...
 *  \param out_err       output the error information, 0 -- no error, 1 -- an error exists
**/
DLLEXPORT void KIRpong_Idv_GetNumHaplo(int *model, int *idx,
	int *out_NumHaplo, int *out_NumSNP, LongBool *out_err)
{
	CORETRY
		_Check_KIRpong_Model(*model);
		CAttrBag_Model *AB = _KIRpong_MODELS_[*model];
 ...
 *  \param out_err       output the error information, 0 -- no error, 1 -- an error exists
**/
DLLEXPORT void KIRpong_Classifier_GetHaplos(int *model, int *idx,
 ...
 *  \param out_err       output the error information, 0 -- no error, 1 -- an error exists
**/
DLLEXPORT void KIRpong_Confusion(int *n_hla, double *init_mat,
...
**/
DLLEXPORT void KIRpong_GetParam(int *EM_MaxNum, double *EM_RelTol, int *Search_MaxNum)
{
	*EM_MaxNum = EM_MaxNum_Iterations;
	*EM_RelTol = EM_FuncRelTol;
	*Search_MaxNum = KIRpong_MAXNUM_SNP_IN_CLASSIFIER;
}
 ...
 *  \param Search_MaxNum    The max number of SNP markers in an individual classifier
**/
DLLEXPORT void KIRpong_SetParam(int *EM_MaxNum, LongBool *If_EM_MaxNum,
 ...
 *  \param out_err       output the error information, 0 -- no error, 1 -- an error exists
**/
DLLEXPORT void KIRpong_BEDFlag(char **bedfn, int *out_SNPOrder, LongBool *out_err)
{
	CORETRY
 ...
 *  \param out_err       output the error information, 0 -- no error, 1 -- an error exists
**/
DLLEXPORT void KIRpong_ConvBED(char **bedfn, int *n_samp, int *n_snp, int *n_save_snp,
...
**/
DLLEXPORT void KIRpong_GPU_Init(char **lib_fn, int *prec, int *out_err)
{
	CORETRY
...
**/
DLLEXPORT void KIRpong_ErrMsg(char **Msg)
{
...
// -----------------------------------------------------------------------
//
// KIRpong: an attribute bagging method
//

/// initialize the package
DLLEXPORT void KIRpong_Init(int *SSE)
{
	memset((void*)_KIRpong_MODELS_, 0, sizeof(_KIRpong_MODELS_));

	#ifdef KIRpong_SSE_OPTIMIZE_HAMMING_DISTANCE
		*SSE = 1;
	#else
		*SSE = 0;
	#endif
...
DLLEXPORT void KIRpong_Done()
{
	try {

	#ifdef KIRpong_ALLOW_GPU_SUPPORT
		Done_GPU_Support();
	#endif

		for (int i=0; i < MODEL_NUM_LIMIT; i++)
		{
			CAttrBag_Model* m = _KIRpong_MODELS_[i];
			_KIRpong_MODELS_[i] = NULL;
			try {
				delete m;
			} catch(...) {}
		}
